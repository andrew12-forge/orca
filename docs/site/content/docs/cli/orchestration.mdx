...
- Every group except `@worktree:<id>` means the live Dispatches of the sender's own Run, each addressed as `dispatch:<id>`. A sender in no Run is refused; use `run:<id>` or `dispatch:<id>`.
